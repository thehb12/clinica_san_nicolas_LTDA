...
const buscarNoRenovacion = async (e) => {
    ...
    if (e.target.querySelector("#cedula").value.length > 7 && contadorData > 10) {
    ...
    if (validarCampoCodigo()) {
        let data = await utilExeFetch('/administrativos/notificacion/no/renovacion/obtener_cedula', forma);
...
const abrirEnlace = (id) => {
    window.open(`/administrativos/notificacion/no/renovacion/generar_pdf/${id}`);
};
